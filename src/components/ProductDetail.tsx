...
export default function ProductDetail({ product, isOpen, onClose, onAddToCart, currentUser, isPageMode = false }: ProductDetailProps) {
  ...
  const isEvent = product.category === "Events & Lifestyle" && product.eventDetails;
  const isFoodAndDrinks = product.category === "Food & Drinks";
  const isDataSubscriptions = product.category === "Data Subscriptions";

  React.useEffect(() => {
    if (isEvent && product.eventDetails?.ticketTiers?.length) {
      setSelectedTier(product.eventDetails.ticketTiers[0]);
    }
    if (isDataSubscriptions && product.menuItems?.length) {
      setSelectedSubPlan(product.menuItems[0]);
    } else {
              ...
              <div className={cn("flex-1 flex flex-col", isPageMode ? "p-0 mt-4" : "p-6 sm:p-10")}>
                <div className="mb-8">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    {isEvent && product.eventDetails?.location && (
                ...
                {product.category !== "Food & Drinks" && product.category !== "Data Subscriptions" && (
                  <div className="space-y-4 mb-8">
                    {/* Main Price & Stock Row */}
                    <div className={cn("grid gap-4", product.type === "good" ? "grid-cols-2" : "grid-cols-1")}>
                      <div className="p-5 bg-gradient-to-br from-slate-50 to-white dark:from-slate-800/40 dark:to-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-sm flex flex-col justify-center">
                        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                          ...
                          {product.discountPercent && product.discountPercent > 0 ? (
                            <span className="px-1.5 py-0.5 bg-red-500 text-white rounded text-[8px] font-black tracking-wider uppercase leading-none animate-bounce">
                        ...
                        <div className="flex items-baseline gap-2 flex-wrap">
                          <p className="text-3xl font-black text-slate-900 dark:text-white leading-none">
                            ...
                            {product.type === "service" && product.pricingType && product.pricingType.toLowerCase() !== 'fixed' && (
                              <span className="text-sm font-bold text-slate-400 dark:text-slate-500 ml-1.5 uppercase">/ {product.pricingType.toLowerCase()}</span>
                            )}
                          </p>
                          {product.priceBefore ? (
                      ...
                      {product.type === "good" && (
                        <div className="p-5 bg-gradient-to-br from-slate-50 to-white dark:from-slate-800/40 dark:to-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-sm">
                    ...
                    <div className="p-5 bg-orange-50/20 dark:bg-zinc-850/20 rounded-2xl border border-orange-100/50 dark:border-zinc-800/80 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        <div className="space-y-1">
                          ...
                          <p className="font-extrabold text-slate-800 dark:text-slate-200">₦{product.price.toLocaleString()}</p>
                        </div>

                        {product.priceBefore ? (
                          <div className="space-y-1">
                            <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Original Price / Price Before</p>
                            <p className="font-bold text-slate-400 line-through">₦{product.priceBefore.toLocaleString()}</p>
                          </div>
                        ) : null}

                        {product.discountPercent && product.discountPercent > 0 ? (
                          <div className="space-y-1">
                            <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Discount Offer</p>
                            <p className="font-extrabold text-red-500 dark:text-red-400">-{product.discountPercent}% OFF Savings</p>
                          </div>
                        ) : null}
                ...
                {product.category === "Food & Drinks" && product.menuItems && product.menuItems.length > 0 && (
                  <div id="food-menu-section" className="space-y-4 mb-8 pt-4 scroll-mt-24">
                    <div className="flex items-center justify-between">
                      ...
                      {!isEvent && product.menuItems && product.menuItems.length > 0 && currentUser?.uid !== product.sellerId && (
                        <motion.div
                          initial={false}
                ...
                {isEvent ? (
                  ...
                  isDataSubscriptions && product.menuItems && product.menuItems.length > 0 ? (
